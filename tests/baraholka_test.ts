...
import { download, parse } from "../providers/baraholka.ts";
import { AdItem } from "../types.ts";

Deno.test("baraholka", async (t) => {
  let _data: string;
  let _items: AdItem[];

  await t.step("download", async () => {
    _data = args.useCache
      ? await cache(".data/baraholka.html", download)
      : await download();
  ...
  await t.step("items", () => {
    ...
    assertStrictEquals(item.provider, "baraholka.onliner.by");
